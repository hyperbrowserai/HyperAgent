...
4. How `page.perform()` (`executeSingleAction`) and `page.ai()` (`executeAction`/`runAgentTask`) flow through CDP.
...
## 0.7. Entry Points: `page.ai()` vs `page.perform()`

> **Deprecation Notice:** `page.aiAction()` is deprecated and remains available as an alias for backwards compatibility. Please use `page.perform()` in new code.
