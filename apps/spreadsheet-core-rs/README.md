...
- Formula recalculation (`SUM`, `AVERAGE`, `MIN`, `MAX`, `COUNT`, `COUNTIF`, `COUNTIFS`, `SUMIF`, `SUMIFS`, `AVERAGEIF`, `AVERAGEIFS`, direct refs, arithmetic refs, `IF`, `IFERROR`, `CHOOSE`, `AND`/`OR`/`NOT`, `CONCAT`/`CONCATENATE`, `LEN`, `LEFT`/`RIGHT`, `UPPER`/`LOWER`/`TRIM`, `ISBLANK`/`ISNUMBER`/`ISTEXT`, `TODAY`, `DATE`, `YEAR`/`MONTH`/`DAY`, `VLOOKUP` exact-match mode, `HLOOKUP` exact-match mode, `XLOOKUP` exact-match mode, `MATCH` exact-match mode, `INDEX`)
- XLSX import/export
