...
- Formula recalculation (`SUM`, `AVERAGE`, `MIN`, `MAX`, `COUNT`, `COUNTIF`, `COUNTIFS`, `SUMIF`, `SUMIFS`, `MINIFS`, `MAXIFS`, `AVERAGEIF`, `AVERAGEIFS`, direct refs, arithmetic refs, `IF`, `IFERROR`, `CHOOSE`, `TRUE`, `FALSE`, `AND`/`OR`/`XOR`/`NOT`, `CONCAT`/`CONCATENATE`, `LEN`, `PI`, `ABS`, `LN`, `LOG10`, `SIN`/`COS`/`TAN`, `ASIN`/`ACOS`/`ATAN`/`ATAN2`, `ROUND`/`ROUNDUP`/`ROUNDDOWN`, `CEILING`/`FLOOR`, `SQRT`, `POWER`, `MOD`, `SIGN`, `INT`, `TRUNC`, `EXACT`, `LEFT`/`RIGHT`/`MID`, `REPT`, `REPLACE`, `SUBSTITUTE`, `VALUE`, `N`, `T`, `CHAR`, `CODE`, `UNICHAR`, `UNICODE`, `SEARCH`, `FIND`, `UPPER`/`LOWER`/`TRIM`, `ISBLANK`/`ISNUMBER`/`ISTEXT`/`ISEVEN`/`ISODD`, `TODAY`, `NOW`, `DATE`, `YEAR`/`MONTH`/`DAY`, `HOUR`/`MINUTE`/`SECOND`, `VLOOKUP` exact-match mode, `HLOOKUP` exact-match mode, `XLOOKUP` exact-match mode, `MATCH` exact-match mode, `INDEX`)
- XLSX import/export
