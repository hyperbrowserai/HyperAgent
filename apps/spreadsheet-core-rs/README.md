...
- Formula recalculation (`SUM`, `AVERAGE`, `MIN`, `MAX`, `COUNT`, direct refs, arithmetic refs, `IF`, `AND`/`OR`/`NOT`, `CONCAT`/`CONCATENATE`, `LEN`, `LEFT`/`RIGHT`, `TODAY`, `DATE`, `YEAR`/`MONTH`/`DAY`, `VLOOKUP` exact-match mode, `XLOOKUP` exact-match mode)
- XLSX import/export
