...
`/v1/openapi` mirrors these workflows with machine-readable path summaries for health + spec discovery endpoints, wizard discovery/run endpoints, agent cache lifecycle endpoints, DuckDB query execution, and workbook event streaming, plus endpoint-catalog extensions (`x-endpoint-catalog-openapi-fingerprint`, `x-endpoint-catalog-openapi-stats`, `x-endpoint-catalog-contract-version`, `x-endpoint-catalog-issue-reason-keys`, `x-endpoint-catalog-issue-reason-descriptions`) for lightweight schema-sync checks.
...
`/v1/workbooks/{id}/agent/schema` also advertises `health_endpoint`, `openapi_endpoint`, `endpoint_openapi_paths` (schema endpoint-key to normalized OpenAPI path), `endpoint_http_methods` (schema endpoint-key to HTTP method list), `endpoint_summaries` (schema endpoint-key to OpenAPI summary string), `endpoint_openapi_operations` (schema endpoint-key to `{ path, methods, summary }` object), `endpoint_catalog_openapi_fingerprint` (sha256 of normalized OpenAPI operation tuples), `endpoint_catalog_openapi_stats` (OpenAPI path/operation summary counters), `endpoint_catalog_contract_version` (stable version token for endpoint-catalog metadata contract), `endpoint_catalog_issue_reason_keys` (stable issue-reason taxonomy for triage tooling), `endpoint_catalog_issue_reason_descriptions` (reason-key to human-readable diagnostics), `endpoint_catalog_coverage` + `endpoint_catalog_diagnostics` (high-level endpoint metadata health counters), `agent_ops_request_shape`, workbook import/export and DuckDB-query endpoint metadata (including `x-export-meta` header shape, import formula-metric response fields, DuckDB query request/response schema + validation codes, formula capability metadata with both summary strings and structured lists, and a workbook event-shape catalog for `workbook.created`, `sheet.added`, `cells.updated`, `formula.recalculated`, `chart.updated`, `workbook.imported`, and `workbook.exported`) for agent discoverability.
`/v1/agent/wizard/schema` includes `health_endpoint`, `openapi_endpoint`, `endpoint_openapi_paths`, `endpoint_http_methods`, `endpoint_summaries`, `endpoint_openapi_operations`, `endpoint_catalog_openapi_fingerprint`, `endpoint_catalog_openapi_stats`, `endpoint_catalog_contract_version`, `endpoint_catalog_issue_reason_keys`, `endpoint_catalog_issue_reason_descriptions`, `endpoint_catalog_coverage`, `endpoint_catalog_diagnostics`, run/import response-shape metadata plus formula capability metadata (summary + structured lists + validation error codes), signature error codes, generic `agent/ops` execute + preview endpoint metadata, `agent_ops_request_shape` + preview request/response shape metadata, top-level `agent_ops_response_shape` metadata, cache endpoint metadata across stats/list/detail/prefix/clear/replay/reexecute/cleanup workflows, cache query/request/response shape contracts (including entries/prefixes age/paging filters), cache validation error codes, structured operation-error payload shape (`agent_ops_result_error_shape`), and DuckDB query request/response schema + validation codes so agent callers can discover wizard import metric fields and supported execution contracts without trial calls.
...
```json
{
  ...
  "endpoint_summaries": {
    "endpoint": "AI-friendly multi-operation endpoint (supports create_sheet/duckdb_query/export_workbook ops)",
    "openapi_endpoint": "Get OpenAPI specification document",
    "agent_ops_preview_endpoint": "Preview and sign a provided agent operation plan without execution"
  },
  "endpoint_openapi_operations": {
    "endpoint": {
      "path": "/v1/workbooks/{id}/agent/ops",
      "methods": ["POST"],
      "summary": "AI-friendly multi-operation endpoint (supports create_sheet/duckdb_query/export_workbook ops)"
    }
  },
  "endpoint_catalog_coverage": {
    "total": 3,
    "method_operation_backed": 3,
    "summary_operation_backed": 3,
    "path_operation_backed": 3,
    "operation_fallback": 0,
  ...
  "endpoint_catalog_openapi_fingerprint": "c9b0d7f6e6ca2fa9f3d3f5f6ab7e7f50df2e8f39b7f8bb9c4d3f1093e2da56f1",
  "endpoint_catalog_openapi_stats": {
    "path_count": 25,
    "operation_count": 29,
    "summarized_operation_count": 29
  },
  "endpoint_catalog_contract_version": "2026-02-endpoint-catalog-v1",
  ...
  "endpoint_catalog_issue_reason_descriptions": {
    "unmapped_methods": "No OpenAPI-backed HTTP method metadata was found for the endpoint key.",
    "method_mismatch": "Schema-advertised methods differ from OpenAPI-derived methods.",
    "summary_mismatch": "Schema-advertised summary differs from OpenAPI-derived summary.",
    "path_mismatch": "Schema-advertised path differs from derived/normalized endpoint path.",
    "method_fallback": "Methods are currently sourced from schema or fallback metadata instead of operation metadata.",
    "summary_fallback": "Summary is currently sourced from schema or fallback metadata instead of operation metadata.",
    "path_fallback": "Path is currently sourced from schema or fallback metadata instead of operation metadata."
  }
}
```
