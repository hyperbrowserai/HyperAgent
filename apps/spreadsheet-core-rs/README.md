...
- `GET /v1/workbooks/{id}/agent/ops/cache/prefixes?request_id_prefix=scenario-&limit=8&max_age_seconds=3600` (prefix suggestions with counts for filter UX; optional **non-blank** request-id prefix + age filters; includes `total_prefixes`, `unscoped_total_prefixes`, echoed `request_id_prefix`, scoped `cutoff_timestamp`, and per-prefix `newest_request_id` + `newest_cached_at`)
- `POST /v1/workbooks/{id}/agent/ops/cache/clear`
