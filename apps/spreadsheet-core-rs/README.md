...
`/v1/agent/wizard/schema` includes run/import response-shape metadata plus formula capability metadata (summary + structured lists + validation error codes), signature error codes, cache endpoint metadata across stats/list/detail/prefix/clear/replay/reexecute/cleanup workflows, cache query/request/response shape contracts (including entries/prefixes age/paging filters), cache validation error codes, structured operation-error payload shape (`agent_ops_result_error_shape`), and DuckDB query request/response schema + validation codes so agent callers can discover wizard import metric fields and supported execution contracts without trial calls.
