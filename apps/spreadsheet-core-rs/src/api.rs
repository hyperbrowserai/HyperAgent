...
async fn get_agent_schema(
  ...
  Ok(Json(json!({
    ...
    "formula_capabilities": {
      "supported_functions": "SUM, AVERAGE, MIN, MAX, COUNT, COUNTIF, SUMIF, AVERAGEIF, direct references, arithmetic expressions, IF, AND, OR, NOT, CONCAT, CONCATENATE, LEN, LEFT, RIGHT, TODAY, DATE, YEAR, MONTH, DAY, VLOOKUP exact-match mode, XLOOKUP exact-match mode",
  ...
  async fn should_expose_cache_and_signature_metadata_in_agent_schema() {
    ...
    assert_eq!(
      schema
        ...
        .and_then(serde_json::Value::as_str),
      Some(
        "SUM, AVERAGE, MIN, MAX, COUNT, COUNTIF, SUMIF, AVERAGEIF, direct references, arithmetic expressions, IF, AND, OR, NOT, CONCAT, CONCATENATE, LEN, LEFT, RIGHT, TODAY, DATE, YEAR, MONTH, DAY, VLOOKUP exact-match mode, XLOOKUP exact-match mode",
      ),
    );
    assert_eq!(
      schema
