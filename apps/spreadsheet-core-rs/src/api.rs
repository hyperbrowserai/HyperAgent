...
async fn get_agent_wizard_schema() -> Json<serde_json::Value> {
  Json(json!({
    ...
    "formula_capabilities": {
      "supported_functions": "SUM, AVERAGE, MIN, MAX, COUNT, COUNTIF, COUNTIFS, SUMIF, SUMIFS, AVERAGEIF, AVERAGEIFS, direct references, arithmetic expressions, IF, AND, OR, NOT, CONCAT, CONCATENATE, LEN, LEFT, RIGHT, UPPER, LOWER, TRIM, TODAY, DATE, YEAR, MONTH, DAY, VLOOKUP exact-match mode, XLOOKUP exact-match mode, MATCH exact-match mode, INDEX",
  ...
  async fn should_expose_cache_and_signature_metadata_in_agent_schema() {
    ...
    assert_eq!(
      schema
        ...
        .and_then(serde_json::Value::as_str),
      Some(
        "SUM, AVERAGE, MIN, MAX, COUNT, COUNTIF, COUNTIFS, SUMIF, SUMIFS, AVERAGEIF, AVERAGEIFS, direct references, arithmetic expressions, IF, AND, OR, NOT, CONCAT, CONCATENATE, LEN, LEFT, RIGHT, UPPER, LOWER, TRIM, TODAY, DATE, YEAR, MONTH, DAY, VLOOKUP exact-match mode, XLOOKUP exact-match mode, MATCH exact-match mode, INDEX",
      ),
    );
    assert_eq!(
      schema
