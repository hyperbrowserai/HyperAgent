use crate::{
    error::ApiError,
    models::{
        AgentOperation, AgentOperationResult, AgentOpsCacheEntriesResponse, AgentOpsCacheEntry,
        AgentOpsCacheEntryDetailResponse, AgentOpsCachePrefix, AgentOpsCachePrefixesResponse,
        AgentOpsCacheStatsResponse, AgentOpsPreviewRequest, AgentOpsPreviewResponse,
        AgentOpsRequest, AgentOpsResponse, AgentPresetRunRequest, AgentScenarioRunRequest,
        AgentWizardImportResult, AgentWizardRunJsonRequest, AgentWizardRunResponse, CellMutation,
        ClearAgentOpsCacheResponse, CreateSheetRequest, CreateSheetResponse, CreateWorkbookRequest,
        CreateWorkbookResponse, ExportResponse, GetCellsRequest, GetCellsResponse,
        PreviewRemoveAgentOpsCacheEntriesByPrefixRequest,
        PreviewRemoveAgentOpsCacheEntriesByPrefixResponse, QueryRequest, QueryResponse,
        RecalculateResponse, ReexecuteAgentOpsCacheEntryRequest,
        ReexecuteAgentOpsCacheEntryResponse, RemoveAgentOpsCacheEntriesByPrefixRequest,
        RemoveAgentOpsCacheEntriesByPrefixResponse, RemoveAgentOpsCacheEntryRequest,
        RemoveAgentOpsCacheEntryResponse, RemoveStaleAgentOpsCacheEntriesRequest,
        RemoveStaleAgentOpsCacheEntriesResponse, ReplayAgentOpsCacheEntryRequest,
        ReplayAgentOpsCacheEntryResponse, SetCellsRequest, SetCellsResponse, UpsertChartRequest,
    },
    state::{AppState, AGENT_OPS_CACHE_MAX_ENTRIES},
    store::{get_cells, recalculate_formulas, run_duckdb_query, set_cells},
...
fn endpoint_http_methods_from_openapi(
    ...
    if let Some(path_entries) = openapi_spec.get("paths").and_then(serde_json::Value::as_object) {
        for (path, methods) in path_entries {
            ...
            let mut method_names = method_map
                .keys()
    ...
    let mut endpoint_methods = serde_json::Map::new();
    if let Some(schema_object) = schema.as_object() {
        for (key, value) in schema_object {
            ...
            let methods = method_index.get(normalized).cloned().unwrap_or_default();
            endpoint_methods.insert(key.clone(), json!(methods));
        }
    }
    serde_json::Value::Object(endpoint_methods)
}

fn endpoint_openapi_paths_from_schema(schema: &serde_json::Value) -> serde_json::Value {
    let mut endpoint_paths = serde_json::Map::new();
    if let Some(schema_object) = schema.as_object() {
        for (key, value) in schema_object {
            if key != "endpoint" && !key.ends_with("_endpoint") {
                continue;
            ...
            endpoint_paths.insert(key.clone(), json!(normalized));
        }
    }
    serde_json::Value::Object(endpoint_paths)
}

fn endpoint_summaries_from_openapi(
    schema: &serde_json::Value,
    openapi_spec: &serde_json::Value,
) -> serde_json::Value {
    ...
    if let Some(schema_object) = schema.as_object() {
        for (key, value) in schema_object {
            ...
            let normalized = endpoint.split('?').next().unwrap_or(endpoint);
            let summary = summary_index.get(normalized).cloned().unwrap_or_default();
            endpoint_summaries.insert(key.clone(), json!(summary));
        }
    }
...
fn operations_signature(operations: &[AgentOperation]) -> Result<String, ApiError> {
...
fn validate_expected_operations_signature(
    ...
    if expected.len() != 64 || !expected.chars().all(|ch| ch.is_ascii_hexdigit()) {
        return Err(ApiError::bad_request_with_code(
            ...
            "Expected operations signature must be a 64-character hexadecimal string.".to_string(),
        ));
    }
...
fn ensure_non_empty_operations(operations: &[AgentOperation]) -> Result<(), ApiError> {
...
async fn import_bytes_into_workbook(
    ...
    for sheet_name in &import_result.sheet_names {
        let _ = state
            .register_sheet_if_missing(workbook_id, sheet_name)
            .await?;
    }
...
async fn import_workbook(
    ...
    let import_result = import_bytes_into_workbook(&state, workbook.id, &bytes, "import").await?;
...
async fn run_agent_wizard(
    ...
    let stop_on_error = parse_optional_bool(stop_on_error_raw, "stop_on_error")?.unwrap_or(true);
    let include_file_base64 =
        parse_optional_bool(include_file_base64_raw, "include_file_base64")?.unwrap_or(false);
    ...
    let import_result = match maybe_file_bytes {
        Some(bytes) => {
            Some(import_bytes_into_workbook(&state, workbook.id, &bytes, actor.as_str()).await?)
        }
        None => None,
    };
    let operations = build_scenario_operations(scenario.as_str(), Some(include_file_base64))?;
...
async fn run_agent_wizard_json(
    ...
    let import_result = match payload.file_base64 {
        Some(encoded_file) => {
            let bytes = BASE64_STANDARD.decode(encoded_file).map_err(|error| {
                ApiError::BadRequest(format!("Field 'file_base64' is not valid base64: {error}",))
            })?;
            Some(import_bytes_into_workbook(&state, workbook.id, &bytes, actor.as_str()).await?)
        }
        None => None,
    };
    let operations = build_scenario_operations(scenario, Some(include_file_base64))?;
    let operation_signature = operations_signature(&operations)?;
    validate_expected_operations_signature(
...
async fn get_wizard_preset_operations(
    ...
    let operations = build_preset_operations(preset.as_str(), query.include_file_base64)?;
    let operation_signature = operations_signature(&operations)?;
    Ok(Json(json!({
      "preset": preset,
      "operations_signature": operation_signature,
...
async fn get_wizard_scenario_operations(
    ...
    let operations = build_scenario_operations(scenario.as_str(), query.include_file_base64)?;
    let operation_signature = operations_signature(&operations)?;
    Ok(Json(json!({
      "scenario": scenario,
      "operations_signature": operation_signature,
...
async fn upsert_chart(
    ...
    state
        .upsert_chart(workbook_id, payload.chart.clone())
        .await?;
    state
        .emit_event(
            workbook_id,
...
fn build_scenario_operations(
    ...
    match scenario {
        ...
        "refresh_and_export" => build_preset_operations("export_snapshot", include_file_base64),
        _ => Err(ApiError::BadRequest(format!(
...
async fn agent_ops_cache_stats(
    ...
    let (
        ...
        .agent_ops_cache_stats(workbook_id, normalized_prefix.as_deref(), cutoff_timestamp)
        .await?;
...
async fn agent_ops_cache_entries(
    ...
    let mapped_entries = entries
        .into_iter()
        .map(
            |(request_id, operations_signature, operation_count, result_count, cached_at)| {
                AgentOpsCacheEntry {
                    request_id,
                    cached_at,
...
fn normalize_sample_limit(
    ...
    sample_limit.unwrap_or(default_limit).max(1).min(max_limit)
}
...
fn normalize_prefix_sort_by(sort_by: Option<&str>) -> Result<String, ApiError> {
    match sort_by.map(str::trim).filter(|value| !value.is_empty()) {
        ...
        Some(value) if value.eq_ignore_ascii_case("count") => Ok("count".to_string()),
        Some(value) if value.eq_ignore_ascii_case("recent") => Ok("recent".to_string()),
        Some(value) if value.eq_ignore_ascii_case("alpha") => Ok("alpha".to_string()),
        Some(value) if value.eq_ignore_ascii_case("span") => Ok("span".to_string()),
...
fn max_age_cutoff_timestamp(
    ...
    Ok(max_age_seconds.map(|value| Utc::now() - ChronoDuration::seconds(value)))
}
...
async fn preview_remove_agent_ops_cache_entries_by_prefix(
    ...
    let (unscoped_matched_entries, _, _) = state
        ...
        .await?;
    let sample_request_ids = entries.into_iter().map(|entry| entry.0).collect::<Vec<_>>();
...
async fn run_agent_preset(
    ...
    state.get_workbook(workbook_id).await?;
    let operations = build_preset_operations(preset.as_str(), payload.include_file_base64)?;
    let operation_signature = operations_signature(&operations)?;
    validate_expected_operations_signature(
        payload.expected_operations_signature.as_deref(),
        operation_signature.as_str(),
    ...
    let request_id = payload.request_id.clone();
    let actor = payload.actor.unwrap_or_else(|| format!("preset:{preset}"));
    let stop_on_error = payload.stop_on_error.unwrap_or(true);
...
async fn run_agent_scenario(
    ...
    state.get_workbook(workbook_id).await?;
    let operations = build_scenario_operations(scenario.as_str(), payload.include_file_base64)?;
    let operation_signature = operations_signature(&operations)?;
    validate_expected_operations_signature(
        payload.expected_operations_signature.as_deref(),
        operation_signature.as_str(),
...
async fn duckdb_query(
    ...
    let db_path = state.db_path(workbook_id).await?;
    let query_result = run_duckdb_query(&db_path, payload.sql.as_str(), payload.row_limit)?;
...
async fn openapi() -> Json<serde_json::Value> {
    Json(json!({
      ...
      "paths": {
        ...
        "/v1/openapi": {"get": {"summary": "Get OpenAPI specification document"}},
      ...
      "/v1/workbooks/{id}/agent/ops": {"post": {"summary": "AI-friendly multi-operation endpoint (supports create_sheet/duckdb_query/export_workbook ops)"}},
        ...
        "/v1/workbooks/{id}/duckdb/query": {"post": {"summary": "Run read-only DuckDB SELECT/WITH query against workbook tables"}},
...
mod tests {
    use axum::{
        body::to_bytes, extract::Path, extract::Query, extract::State, http::header,
    ...
    use crate::{
        ...
        fixture_corpus::{
            ...
            COMPAT_BASELINE_FILE_NAME, COMPAT_DEFAULT_CACHED_FORMULA_FILE_NAME,
            COMPAT_ERROR_CACHED_FORMULA_FILE_NAME, COMPAT_FORMULA_MATRIX_FILE_NAME,
            COMPAT_FORMULA_ONLY_DUAL_FILE_NAME, COMPAT_FORMULA_ONLY_NORMALIZED_FILE_NAME,
            COMPAT_FORMULA_ONLY_OFFSET_NORMALIZED_FILE_NAME, COMPAT_FORMULA_ONLY_SHEET_FILE_NAME,
        ...
        models::{
            ...
            ReplayAgentOpsCacheEntryRequest, SetCellsRequest,
        },
        state::{AppState, AGENT_OPS_CACHE_MAX_ENTRIES},
    ...
    fn assert_schema_endpoint_aliases(
        ...
        assert_eq!(
            ...
            Some(vec!["GET"]),
            "{schema_name} should expose GET method metadata for health endpoint",
        );
        assert_eq!(
            schema
                .get("endpoint_http_methods")
            ...
            Some(vec!["GET"]),
            "{schema_name} should expose GET method metadata for openapi endpoint",
        );
        assert_eq!(
            schema
            ...
            "{schema_name} should expose health endpoint summary metadata",
        );
        assert_eq!(
            schema
                .get("endpoint_summaries")
            ...
            "{schema_name} should expose openapi endpoint summary metadata",
        );
        assert_eq!(
            schema
            ...
            "{schema_name} should expose normalized endpoint_openapi_paths for endpoint",
        );
        assert_eq!(
            schema
                .get("endpoint_openapi_paths")
                ...
                .and_then(serde_json::Value::as_str),
            Some("/health"),
            "{schema_name} should expose normalized endpoint_openapi_paths for health endpoint",
        );
        assert_eq!(
            schema
                .get("endpoint_openapi_paths")
    ...
    fn endpoint_method_list_from_openapi_path(
    ...
    fn parse_advertised_endpoint_methods(value: &serde_json::Value) -> Vec<String> {
        let mut methods = value
            .as_array()
            .expect("endpoint_http_methods entries should be arrays")
            .iter()
            .filter_map(serde_json::Value::as_str)
            .map(|value| value.trim().to_ascii_uppercase())
            .filter(|value| !value.is_empty())
    ...
    fn assert_endpoint_openapi_paths_match_schema_endpoints(
        ...
        assert_eq!(
            endpoint_openapi_paths.len(),
            endpoint_entries.len(),
            "{schema_name} endpoint_openapi_paths should cover every endpoint field",
        );

        for (key, endpoint) in endpoint_entries {
            let normalized = endpoint.split('?').next().unwrap_or(endpoint);
            let advertised_path = endpoint_openapi_paths
                .get(key)
                .and_then(serde_json::Value::as_str)
                .expect("endpoint_openapi_paths entry should be string");
            assert_eq!(
                advertised_path,
                normalized,
                "{schema_name} endpoint_openapi_paths for '{key}' should match normalized endpoint path",
            );
            assert!(
                advertised_path.starts_with('/'),
                "{schema_name} endpoint_openapi_paths for '{key}' should start with '/'",
            );
            assert_eq!(
                advertised_path,
                advertised_path.trim(),
                "{schema_name} endpoint_openapi_paths for '{key}' should be trimmed",
            );
        }
    }

    fn assert_endpoint_openapi_paths_match_openapi_paths(
        schema: &serde_json::Value,
        schema_name: &str,
        paths: &serde_json::Map<String, serde_json::Value>,
    ) {
        let endpoint_openapi_paths = schema
            ...
            .and_then(serde_json::Value::as_object)
            .expect("schema should expose endpoint_openapi_paths object");
        for (key, value) in endpoint_openapi_paths {
            let path = value
                .as_str()
                .expect("endpoint_openapi_paths entries should be strings");
            assert!(
    ...
    fn endpoint_summary_from_openapi_path(
        paths: &serde_json::Map<String, serde_json::Value>,
        path: &str,
    ) -> String {
        let method_map = paths
            .get(path)
            .and_then(serde_json::Value::as_object)
            .expect("openapi endpoint entry should be an object");
        let mut summaries = method_map
            .iter()
            .filter_map(|(method, metadata)| {
                let summary = metadata
                    .get("summary")
    ...
    fn assert_endpoint_summaries_match_openapi_paths(
        schema: &serde_json::Value,
        schema_name: &str,
        paths: &serde_json::Map<String, serde_json::Value>,
    ) {
        ...
        let endpoint_summaries = schema
            ...
            .expect("schema should expose endpoint_summaries object");
        assert_eq!(
            endpoint_summaries.len(),
            endpoint_entries.len(),
            "{schema_name} endpoint_summaries should cover every endpoint field",
        );

        for (key, endpoint) in endpoint_entries {
            let normalized = endpoint.split('?').next().unwrap_or(endpoint);
            let advertised_summary = endpoint_summaries
                .get(key)
                .and_then(serde_json::Value::as_str)
                .expect("endpoint_summaries entry should be a string");
            let expected_summary = endpoint_summary_from_openapi_path(paths, normalized);
            assert_eq!(
                advertised_summary, expected_summary,
                "{schema_name} endpoint_summaries for '{key}' should match openapi path '{normalized}'",
            );
            assert!(
                !advertised_summary.trim().is_empty(),
                "{schema_name} endpoint_summaries for '{key}' should not be blank",
            );
        }
    }
    ...
    fn assert_endpoint_summaries_map_is_normalized(
        endpoint_summaries: &serde_json::Value,
        schema_name: &str,
    ) {
        let entries = endpoint_summaries
            .as_object()
            .expect("endpoint_summaries should be an object");
        assert!(
            !entries.is_empty(),
            "{schema_name} endpoint_summaries should not be empty",
        );
        for (key, value) in entries {
            let summary = value
                .as_str()
                .expect("endpoint_summaries values should be strings");
            assert!(
                !summary.trim().is_empty(),
                "{schema_name} endpoint_summaries entry '{key}' should not be blank",
            );
            assert_eq!(
                summary,
                summary.trim(),
                "{schema_name} endpoint_summaries entry '{key}' should be trimmed",
            );
        }
    }
    ...
    fn assert_endpoint_http_methods_match_openapi_paths(
        ...
        let endpoint_http_methods = schema
            ...
            .expect("schema should expose endpoint_http_methods object");
        assert_eq!(
            endpoint_http_methods.len(),
            endpoint_entries.len(),
            "{schema_name} endpoint_http_methods should cover every endpoint field",
        );

        for (key, endpoint) in endpoint_entries {
            let normalized = endpoint.split('?').next().unwrap_or(endpoint);
            let advertised_methods = endpoint_http_methods
                .get(key)
                .map(parse_advertised_endpoint_methods)
                .unwrap_or_default();
            let expected_methods = endpoint_method_list_from_openapi_path(paths, normalized);
    ...
    fn workbook_import_default_cached_formula_fixture_bytes() -> Vec<u8> {
        workbook_fixture_bytes(COMPAT_DEFAULT_CACHED_FORMULA_FILE_NAME)
    }
    ...
    fn workbook_import_formula_only_sheet_fixture_bytes() -> Vec<u8> {
        workbook_fixture_bytes(COMPAT_FORMULA_ONLY_SHEET_FILE_NAME)
    }
    ...
    fn workbook_import_formula_only_dual_fixture_bytes() -> Vec<u8> {
        workbook_fixture_bytes(COMPAT_FORMULA_ONLY_DUAL_FILE_NAME)
    }
    ...
    fn should_parse_optional_boolean_fields() {
        assert_eq!(
            parse_optional_bool(Some("true".to_string()), "flag").expect("true should parse"),
            Some(true),
        );
        assert_eq!(
            parse_optional_bool(Some("0".to_string()), "flag").expect("0 should parse"),
            Some(false),
        );
        assert_eq!(
    ...
    async fn should_trim_formula_inputs_in_set_cells_batch() {
        ...
        let snapshot = load_sheet_snapshot(&db_path, "Sheet1").expect("sheet snapshot should load");
    ...
    #[tokio::test]
    async fn should_execute_read_only_duckdb_query_for_workbook_cells() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some("duckdb-query-read-only".to_string()))
            .await
            .expect("workbook should be created");

        let _ = set_cells_batch(
            State(state.clone()),
            Path(workbook.id),
            Json(SetCellsRequest {
                sheet: "Sheet1".to_string(),
        ...
        .await
        .expect("cells should be inserted");

        let query_response = duckdb_query(
      State(state),
      Path(workbook.id),
      Json(QueryRequest {
        sql: "SELECT sheet, row_index, col_index, raw_value, formula, evaluated_value FROM cells ORDER BY row_index, col_index".to_string(),
        row_limit: Some(10),
      }),
    )
    .await
    ...
    .0;
        ...
        assert_eq!(query_response.row_limit, 10);
        assert_eq!(query_response.row_count, 2);
        ...
        assert_eq!(query_response.rows[1][0], Some("Sheet1".to_string()));
        assert_eq!(query_response.rows[1][1], Some("1".to_string()));
        assert_eq!(query_response.rows[1][2], Some("2".to_string()));
        assert_eq!(
            query_response.rows[1][4].as_deref(),
            Some("=SUM(A1:A1)"),
            "formula column should preserve canonical formula text",
        );
        let evaluated_numeric_value = query_response.rows[1][5]
            .as_deref()
            .and_then(|value| value.parse::<f64>().ok());
        assert_eq!(
    ...
    #[tokio::test]
    async fn should_reject_non_read_only_duckdb_queries() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some("duckdb-query-reject-write".to_string()))
            .await
            .expect("workbook should be created");
        ...
        .await
        .expect_err("non-read-only sql should fail");
        ...
        match error {
            ApiError::BadRequestWithCode { code, message } => {
                ...
                assert!(
                    ...
                    "error message should explain read-only query requirements",
                );
            }
  ...
  async fn should_allow_duckdb_query_with_leading_comment() {
    ...
    async fn should_reject_non_positive_duckdb_query_row_limit() {
        ...
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some("duckdb-query-invalid-row-limit".to_string()))
            .await
            .expect("workbook should be created");
        ...
        let error = duckdb_query(
            ...
            Path(workbook.id),
            Json(QueryRequest {
                sql: "SELECT * FROM cells".to_string(),
                row_limit: Some(0),
            }),
        )
        .await
        .expect_err("zero row_limit should fail");

        match error {
            ApiError::BadRequestWithCode { code, message } => {
                assert_eq!(code, "INVALID_QUERY_ROW_LIMIT");
                assert!(
                    message.contains("row_limit"),
                    "error message should mention row_limit requirements",
                );
            }
            other => panic!("expected invalid row limit error code, got {other:?}"),
        }
    }

    #[tokio::test]
    async fn should_truncate_duckdb_query_rows_when_limit_is_reached() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some("duckdb-query-row-limit".to_string()))
            .await
            .expect("workbook should be created");
        ...
        let query_response = duckdb_query(
            ...
            Path(workbook.id),
            Json(QueryRequest {
                sql: "SELECT row_index, raw_value FROM cells ORDER BY row_index".to_string(),
                row_limit: Some(1),
            }),
        )
        .await
        ...
        assert_eq!(query_response.row_limit, 1);
        assert_eq!(query_response.row_count, 1);
        assert!(query_response.truncated);
  ...
  async fn should_reject_multi_statement_duckdb_query_sql() {
    ...
    async fn should_import_every_committed_fixture_corpus_via_api_helper() {
        ...
        for (fixture_index, fixture_file_name) in
            workbook_import_fixture_file_names().iter().enumerate()
        {
            let workbook = state
    ...
    async fn should_export_every_committed_fixture_corpus_after_api_import() {
        ...
        .to_string();
            assert_eq!(
                exported_file_name, expected_export_response.file_name,
            ...
            let export_meta_json: serde_json::Value = serde_json::from_str(export_meta)
                .unwrap_or_else(|error| {
                    panic!(
            ...
            assert_eq!(
                export_meta_json, expected_report_json_value,
    ...
    async fn should_roundtrip_every_committed_fixture_corpus_via_api_helpers() {
            ...
            workbook_import_fixture_file_names().iter().enumerate()
        {
            ...
            .await
            .unwrap_or_else(|error| {
                panic!("fixture {fixture_file_name} should import into source workbook: {error:?}")
            });
            assert!(
    ...
    async fn should_report_comprehensive_normalized_formula_metrics_on_import() {
        ...
        let workbook = state
            .create_workbook(Some(
                "import-comprehensive-normalization-fixture".to_string(),
            ))
            .await
            .expect("workbook should be created");
        let mut events = state
    ...
    #[tokio::test]
    async fn should_report_default_cached_formula_import_metrics_on_import() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some("import-no-cached-formula-fixture".to_string()))
            .await
            .expect("workbook should be created");
        let mut events = state
            .subscribe(workbook.id)
        ...
        let import_result = import_bytes_into_workbook(
            &state,
            workbook.id,
            &workbook_import_default_cached_formula_fixture_bytes(),
            "default-cached-formula-import",
        )
        .await
        .expect("no-cached formula fixture import should succeed");

        assert_eq!(import_result.formula_cells_imported, 1);
        assert_eq!(import_result.formula_cells_with_cached_values, 1);
        assert_eq!(import_result.formula_cells_without_cached_values, 0);
    ...
    #[tokio::test]
    async fn should_report_error_cached_formula_import_metrics_on_import() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some("import-error-cached-formula-fixture".to_string()))
            .await
            .expect("workbook should be created");
        let mut events = state
            .subscribe(workbook.id)
        ...
        let import_result = import_bytes_into_workbook(
            &state,
            workbook.id,
            &workbook_import_error_cached_formula_fixture_bytes(),
            "error-cached-formula-import",
        )
        .await
        .expect("error-cached formula fixture import should succeed");
    ...
    #[tokio::test]
    async fn should_report_formula_only_sheet_import_metrics_on_import() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some("import-formula-only-sheet-fixture".to_string()))
            .await
            .expect("workbook should be created");
        let mut events = state
            .subscribe(workbook.id)
        ...
        let import_result = import_bytes_into_workbook(
            &state,
            workbook.id,
            &workbook_import_formula_only_sheet_fixture_bytes(),
            "formula-only-sheet-import",
        )
        .await
        .expect("formula-only sheet fixture import should succeed");
    ...
    #[tokio::test]
    async fn should_report_formula_only_offset_normalized_import_metrics_on_import() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some(
                "import-formula-only-offset-normalized-fixture".to_string(),
            ))
            .await
            .expect("workbook should be created");
        let mut events = state
        ...
        let import_result = import_bytes_into_workbook(
            &state,
            workbook.id,
            &workbook_import_formula_only_offset_normalized_fixture_bytes(),
            "formula-only-offset-normalized-import",
        )
        .await
        .expect("formula-only offset normalized fixture import should succeed");
        ...
        assert!(
            import_result
                ...
                .iter()
                .any(|warning| warning.contains("1 formula(s) were normalized")),
            "import warnings should include formula-only offset normalization telemetry",
        );
    ...
    #[tokio::test]
    async fn should_report_formula_only_dual_import_metrics_on_import() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
            .create_workbook(Some("import-formula-only-dual-fixture".to_string()))
            .await
            .expect("workbook should be created");
        let mut events = state
            .subscribe(workbook.id)
        ...
        let import_result = import_bytes_into_workbook(
            &state,
            workbook.id,
            &workbook_import_formula_only_dual_fixture_bytes(),
            "formula-only-dual-import",
        )
        .await
        .expect("formula-only dual fixture import should succeed");

        assert_eq!(import_result.cells_imported, 2);
        assert_eq!(import_result.formula_cells_imported, 2);
        assert_eq!(import_result.formula_cells_with_cached_values, 0);
        assert_eq!(import_result.formula_cells_without_cached_values, 2);
        assert_eq!(import_result.formula_cells_normalized, 1);
        assert!(
            import_result
                .warnings
                .iter()
                .any(|warning| warning.contains("1 formula(s) were normalized")),
            "dual fixture import warnings should include normalization telemetry",
        );
    ...
    fn should_generate_stable_operation_signatures() {
        ...
        let signature_a = operations_signature(&operations).expect("signature should build");
        let signature_b = operations_signature(&operations).expect("signature should be stable");
    ...
    fn should_validate_expected_operation_signatures() {
        ...
        let signature = operations_signature(&operations).expect("signature should build");
        assert!(
        ...
        assert!(
            validate_expected_operations_signature(Some("mismatch"), signature.as_str()).is_err(),
        ...
        let mismatch_error = validate_expected_operations_signature(
    ...
    fn should_allow_blank_expected_signature_values() {
        ...
        let signature = operations_signature(&operations).expect("signature should build");
        assert!(
            validate_expected_operations_signature(Some("   "), signature.as_str()).is_ok(),
    ...
    fn should_change_signature_when_operations_change() {
        ...
        let with_file = build_scenario_operations("refresh_and_export", Some(true))
            .expect("scenario should build");
        let without_file_signature =
            operations_signature(&without_file).expect("signature should build");
        let with_file_signature = operations_signature(&with_file).expect("signature should build");
        assert_ne!(
    ...
    fn should_reject_request_id_signature_conflicts() {
        ...
        let conflict_error = validate_request_id_signature_consistency(Some("abc"), "def")
    ...
    async fn should_round_trip_cache_stats_and_clear_via_handlers() {
        ...
        let cleared = clear_agent_ops_cache(State(state.clone()), Path(workbook.id))
            .await
    ...
    async fn should_list_cache_entries_from_newest_with_limit() {
        ...
        assert_eq!(
            min_clamped_response.entries[0].request_id,
            "handler-entries-3"
        );
        ...
        assert_eq!(filtered_response.entries[0].request_id, "handler-entries-2",);
    ...
    async fn should_return_cache_entry_detail_via_handler() {
        ...
        let detail =
            agent_ops_cache_entry_detail(State(state), Path((workbook.id, "detail-1".to_string())))
                .await
    ...
    #[tokio::test]
    async fn should_default_stop_on_error_to_true_when_reexecuting_cache_entry() {
        let temp_dir = tempdir().expect("temp dir should be created");
        let state = AppState::new(temp_dir.path().to_path_buf()).expect("state should initialize");
        let workbook = state
        ...
        let _ = agent_ops(
            ...
            Path(workbook.id),
            Json(AgentOpsRequest {
                request_id: Some("source-stop-on-error-default".to_string()),
                actor: Some("test".to_string()),
                stop_on_error: Some(true),
                expected_operations_signature: None,
                operations: vec![
        ...
        let reexecute = reexecute_agent_ops_cache_entry(
            ...
            Path(workbook.id),
            Json(ReexecuteAgentOpsCacheEntryRequest {
                request_id: "source-stop-on-error-default".to_string(),
                new_request_id: Some("reexecute-stop-on-error-default".to_string()),
                actor: Some("test-reexecute".to_string()),
                stop_on_error: None,
                expected_operations_signature: None,
            }),
        ...
        .await
        .expect("default stop_on_error should halt after first failure")
        .0;
    ...
    async fn should_expose_cache_and_signature_metadata_in_agent_schema() {
        ...
        assert_eq!(
            schema
                .get("endpoint_openapi_paths")
                .and_then(|value| value.get("agent_ops_cache_entries_endpoint"))
                .and_then(serde_json::Value::as_str),
            Some("/v1/workbooks/{id}/agent/ops/cache/entries"),
        );
        assert_eq!(
            schema
                .get("endpoint_openapi_operations")
      ...
      Some(
        ...
        assert!(
            ...
            "agent schema should advertise read-only query sql validation code",
        );
        assert!(
            duckdb_query_validation_error_codes.contains(&"INVALID_QUERY_ROW_LIMIT"),
            "agent schema should advertise query row-limit validation code",
        );
        assert_eq!(
            schema
                .get("workbook_import_event_shape")
    ...
    async fn should_expose_response_shapes_in_wizard_schema() {
        ...
        assert_eq!(
            schema
                .get("agent_ops_cache_stats_endpoint")
                .and_then(serde_json::Value::as_str),
            Some("/v1/workbooks/{id}/agent/ops/cache?request_id_prefix=scenario-&max_age_seconds=3600"),
        );
        assert_eq!(
            schema
                .get("agent_ops_cache_entries_endpoint")
                .and_then(serde_json::Value::as_str),
            Some("/v1/workbooks/{id}/agent/ops/cache/entries?request_id_prefix=demo&offset=0&limit=20"),
        );
        assert_eq!(
            schema
                .get("agent_ops_cache_entry_detail_endpoint")
        ...
        assert_eq!(
            schema
                ...
                .and_then(serde_json::Value::as_str),
            Some("optional iso timestamp of oldest cached entry"),
        );
        assert_eq!(
            schema
                .get("agent_ops_cache_entries_response_shape")
    ...
    async fn should_keep_wizard_cache_schema_contracts_in_sync_with_agent_schema() {
        ...
        let agent_cache_keys = agent_schema
            ...
            .keys()
            .filter(|key| key.starts_with("agent_ops_cache_"))
            .cloned()
            .collect::<std::collections::BTreeSet<_>>();
        let wizard_cache_keys = wizard_schema
            .as_object()
            ...
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(
            agent_cache_keys, wizard_cache_keys,
    ...
    async fn should_expose_openapi_paths_for_wizard_and_agent_discovery_endpoints() {
        ...
        let health_summary = paths
            .get("/health")
            .and_then(|value| value.get("get"))
            .and_then(|value| value.get("summary"))
            .and_then(serde_json::Value::as_str)
            .expect("health path should expose get summary");
        assert!(
            health_summary.contains("Health"),
            "health summary should mention health checks",
        );

        let openapi_summary = paths
            .get("/v1/openapi")
            .and_then(|value| value.get("get"))
            .and_then(|value| value.get("summary"))
            .and_then(serde_json::Value::as_str)
            .expect("openapi path should expose get summary");
        assert!(
            openapi_summary.contains("OpenAPI"),
    ...
    async fn should_keep_shared_endpoint_http_methods_in_sync_between_wizard_and_agent_schemas() {
        ...
        let agent_endpoint_openapi_paths = agent_schema
            .get("endpoint_openapi_paths")
            .expect("agent schema should expose endpoint_openapi_paths");
        ...
        let agent_schema_object = agent_schema
            .as_object()
            .expect("agent schema should be object");
        let wizard_schema_object = wizard_schema
            .as_object()
            .expect("wizard schema should be object");
        let shared_endpoint_keys = agent_method_map
            .keys()
            ...
            .collect::<std::collections::BTreeSet<_>>();
        assert!(
            !shared_endpoint_keys.is_empty(),
            "wizard and agent schemas should share endpoint keys with identical endpoint paths",
        );
        assert!(
            shared_endpoint_keys.contains("health_endpoint"),
            "wizard and agent endpoint_http_methods should both include health endpoint method metadata",
        );
        assert!(
            shared_endpoint_keys.contains("openapi_endpoint"),
            "wizard and agent endpoint_http_methods should both include openapi endpoint method metadata",
        );
        assert!(
            shared_endpoint_keys.contains("agent_ops_endpoint"),
            "wizard and agent endpoint_http_methods should both include agent_ops_endpoint method metadata",
        );
        ...
        let agent_summary_map = agent_endpoint_summaries
            .as_object()
            .expect("agent endpoint_summaries should be object");
        let wizard_summary_map = wizard_endpoint_summaries
            .as_object()
            .expect("wizard endpoint_summaries should be object");
        let agent_openapi_path_map = agent_endpoint_openapi_paths
            .as_object()
            .expect("agent endpoint_openapi_paths should be object");
        ...
        for key in shared_endpoint_keys {
            assert!(
                ...
                "wizard and agent endpoint_summaries should expose shared key '{key}'",
            );
            assert_eq!(
                agent_summary_map.get(&key),
                wizard_summary_map.get(&key),
                "wizard and agent endpoint_summaries should stay aligned for shared key '{key}'",
            );
            assert_eq!(
