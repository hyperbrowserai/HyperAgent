...
- Wizard and agent integration panels now include a sortable discovered endpoint catalog (`*_endpoint` + `endpoint`) with one-click copy actions for every advertised schema endpoint, one-click copy actions for normalized OpenAPI paths, one-click copy of the full endpoint-catalog JSON payload (including source + mismatch diagnostics), schema-advertised normalized OpenAPI paths, operation/schema/openapi method + summary + path source badges, operation-metadata fallback warnings, schema-vs-openapi method/summary mismatch warnings, schema-vs-derived path mismatch warnings, explicit unmapped-endpoint warnings, and OpenAPI sync-unavailable notices
