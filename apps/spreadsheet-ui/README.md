...
- Read-only DuckDB query console with tabular result preview (row-limit input is validated and capped to backend max 1000), with direct-run and signed `agent/ops` execution paths
...
- Wizard schema panel now also shows discovered `health` + `openapi` endpoints, `agent/ops` execute + preview endpoints, `agent_ops` execute-request shape, scenario-preview/agent-ops-preview shapes, `agent_ops` response shape metadata, DuckDB query request/response shapes, DuckDB validation error codes, cache stats/list/detail/prefix/clear/replay/reexecute/cleanup endpoints, cache query/request/response shapes, cache validation error codes, and structured operation-error payload fields
- Wizard and agent integration panels include one-click copy actions for discovered `health`/`openapi` plus `agent/ops` execute/preview endpoints
- Wizard and agent integration panels now include a searchable + sortable discovered endpoint catalog (`*_endpoint` + `endpoint`) with one-click copy actions for every advertised schema endpoint, one-click copy actions for normalized OpenAPI paths, one-click copy actions for schema-advertised OpenAPI fingerprints, one-click copy of visible endpoint URL lists, one-click copy of visible normalized OpenAPI path lists, one-click copy of visible method lists, one-click copy of visible summary lists, one-click copy of both visible and full endpoint issue-report JSON payloads for triage handoff (including reason-count breakdowns), one-click copy of visible issue-key payloads, one-click copy of visible issue-URL payloads (annotated with issue reasons), one-click copy of visible issue-path payloads (annotated with issue reasons), one-click copy of visible issue-reason-count payloads, one-click copy of visible issue-summary payloads, one-click copy of issue-reason glossary payloads (keys + descriptions + schema/openapi sync flags), one-click copy of both full and visible (current filter/view-mode scope) endpoint-catalog JSON payloads plus visible endpoint-key payloads, high-level catalog health/status issue counters (mismatch/fallback/unmapped/openapi-sync), summary badges for local status + schema sync/drift state, schema-provided coverage/diagnostics/OpenAPI-stats snapshots, schema-vs-local coverage/diagnostics drift warnings plus explicit in-sync confirmations, schema-vs-live-openapi stats drift warnings, cross-schema fingerprint mismatch warnings, contract-version mismatch warnings against peer schema + `/v1/openapi` extension metadata, issue-reason-key and issue-reason-description sync warnings against peer schema + `/v1/openapi` extension metadata, selectable catalog view modes (`all`, `issues`, `mismatches`, `fallback`, `unmapped`) plus issue-reason filters (`all reasons` + per-reason slices), reason quick-select chips with per-reason counts, reset controls, and visible issue/mismatch/fallback/unmapped counters plus visible issue-reason summaries for rapid triage, schema-advertised normalized OpenAPI paths, operation/schema/openapi method + summary + path source badges, operation-metadata fallback warnings, schema-vs-openapi method/summary mismatch warnings, schema-vs-derived path mismatch warnings, explicit unmapped-endpoint warnings, and OpenAPI sync-unavailable notices
...
- Import summaries (wizard + direct workbook import) show sheet/cell/formula counts, including cached vs uncached and normalized formula-cell coverage when formulas are present
- Wizard run success notices now mirror import-summary telemetry (sheets/cells/formulas with cached/uncached/normalized breakdown) when import metadata is present
...
- Agent integration panel surfaces discovered `health`, `openapi`, `agent/ops` execute, and preview endpoint metadata
...
- Agent demo flow now includes a signed `duckdb_query` op and surfaces sampled row-count feedback in success notices
...
- Latest `request_id` replay now calls dedicated cache replay API (does not require local operation plan state), and replay notices include source cache timestamp + relative-age context
...
- Agent operation result table highlights per-operation `error_code` / `error_message` details when an op fails
...
- Agent integration panel now also displays discovered ops execute-request shape plus ops-preview request/response shapes from schema
...
- Agent integration panel displays discovered `agent_ops_response_shape` metadata from schema for top-level response contract discoverability
...
- Cache controls include optional rerun `request_id` override used by per-entry rerun action (value is client-trimmed; blank-after-trim generates server-side id; conflicting signatures surface `REQUEST_ID_CONFLICT`)
