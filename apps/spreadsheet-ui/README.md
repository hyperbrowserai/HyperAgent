...
- Wizard and agent integration panels now include a sortable discovered endpoint catalog (`*_endpoint` + `endpoint`) with one-click copy actions for every advertised schema endpoint, schema/openapi method-source badges, schema-vs-openapi mismatch warnings, and explicit unmapped-endpoint warnings
