...
- Wizard and agent integration panels now include a sortable discovered endpoint catalog (`*_endpoint` + `endpoint`) with one-click copy actions for every advertised schema endpoint, schema-advertised/OpenAPI-verified HTTP method badges, and explicit unmapped-endpoint warnings
