...
- Wizard schema panel now also shows discovered `agent/ops` execute + preview endpoints, scenario-preview/agent-ops-preview shapes, `agent_ops` response shape metadata, DuckDB query request/response shapes, DuckDB validation error codes, cache stats/list/detail/prefix/clear/replay/reexecute/cleanup endpoints, cache query/request/response shapes, cache validation error codes, and structured operation-error payload fields
