...
interface EndpointCatalogCoverageStats {
  total: number;
...
interface EndpointCatalogOpenApiStats {
  pathCount: number;
  operationCount: number;
  summarizedOperationCount: number;
}
...
interface EndpointCatalogOpenApiStatsDrift {
  hasDrift: boolean;
  pathCount: boolean;
  operationCount: boolean;
  summarizedOperationCount: boolean;
}
...
function buildEndpointCatalogDiagnostics(args: {
  coverage: EndpointCatalogCoverageStats;
  unmappedCount: number;
  isOpenApiSyncAvailable: boolean;
}): EndpointCatalogDiagnostics {
  const mismatchCount =
    args.coverage.methodMismatches
    + args.coverage.summaryMismatches
    + args.coverage.pathMismatches;
  const openApiSyncIssueCount = args.isOpenApiSyncAvailable ? 0 : 1;
  const issueCount =
    mismatchCount
    + args.coverage.operationFallback
    + args.unmappedCount
    + openApiSyncIssueCount;
  const level: EndpointCatalogDiagnostics["level"] = !args.isOpenApiSyncAvailable
    || mismatchCount > 0
    ? "error"
    : issueCount > 0
      ? "warning"
  ...
  return {
    ...
    issueCount,
    mismatchCount,
    fallbackCount: args.coverage.operationFallback,
...
function normalizeEndpointIssueReasonKeys(
  value: unknown,
): EndpointIssueReason[] | null {
  if (!Array.isArray(value)) {
    return null;
  }
  const normalized = value
    .filter((entry): entry is string => typeof entry === "string")
    .map((entry) => entry.trim())
    .filter((entry): entry is EndpointIssueReason =>
      ENDPOINT_ISSUE_REASONS.some((reason) => reason === entry)
    );
...
function normalizeSchemaEndpointCatalogOpenApiStats(
  value: unknown,
): EndpointCatalogOpenApiStats | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null;
  }
  const statsValue = value as Record<string, unknown>;
  const pathCount = statsValue.path_count;
  const operationCount = statsValue.operation_count;
  const summarizedOperationCount = statsValue.summarized_operation_count;
  if (
    typeof pathCount !== "number"
    || typeof operationCount !== "number"
    || typeof summarizedOperationCount !== "number"
  ) {
    return null;
  }
  return {
...
function collectOpenApiStats(spec: unknown): EndpointCatalogOpenApiStats | null {
  if (!spec || typeof spec !== "object" || Array.isArray(spec)) {
    return null;
  }
  const paths = (spec as { paths?: unknown }).paths;
  ...
  let summarizedOperationCount = 0;
  for (const methods of Object.values(paths as Record<string, unknown>)) {
    if (!methods || typeof methods !== "object" || Array.isArray(methods)) {
      continue;
    }
    for (const [method, metadata] of Object.entries(methods as Record<string, unknown>)) {
      if (!method.trim()) {
        continue;
      }
      operationCount += 1;
      const summary = (
        metadata
          ...
          ? (metadata as { summary?: unknown }).summary
          : undefined
      );
      if (typeof summary === "string" && summary.trim()) {
        summarizedOperationCount += 1;
      }
    }
  }
  return {
    pathCount: Object.keys(paths as Record<string, unknown>).length,
    operationCount,
    summarizedOperationCount,
  };
}

function buildEndpointCatalogOpenApiStatsDrift(args: {
  schemaStats: EndpointCatalogOpenApiStats | null;
  openApiStats: EndpointCatalogOpenApiStats | null;
}): EndpointCatalogOpenApiStatsDrift | null {
  if (!args.schemaStats || !args.openApiStats) {
    return null;
  }
  const pathCount = args.schemaStats.pathCount !== args.openApiStats.pathCount;
  const operationCount = args.schemaStats.operationCount !== args.openApiStats.operationCount;
  const summarizedOperationCount =
    args.schemaStats.summarizedOperationCount !== args.openApiStats.summarizedOperationCount;
  return {
    hasDrift: pathCount || operationCount || summarizedOperationCount,
    pathCount,
    operationCount,
    summarizedOperationCount,
  };
...
export function SpreadsheetApp() {
  ...
  const [isCopyingWizardEndpointIssueReport, setIsCopyingWizardEndpointIssueReport] =
    useState(false);
  const [isCopyingVisibleWizardEndpointIssueReport, setIsCopyingVisibleWizardEndpointIssueReport] =
    useState(false);
  ...
  const [isCopyingVisibleAgentIssueSummaries, setIsCopyingVisibleAgentIssueSummaries] =
    useState(false);
  const [isCopyingVisibleAgentIssuePaths, setIsCopyingVisibleAgentIssuePaths] =
    useState(false);
  ...
  const wizardSchemaEndpointOpenApiStats = useMemo(
    () =>
      normalizeSchemaEndpointCatalogOpenApiStats(
        wizardSchemaQuery.data?.endpoint_catalog_openapi_stats,
      ),
    [wizardSchemaQuery.data?.endpoint_catalog_openapi_stats],
  );
  const wizardSchemaIssueReasonKeys = useMemo(
    () =>
  ...
  const openApiSpecIssueReasonKeys = useMemo(
    () =>
      normalizeEndpointIssueReasonKeys(
  ...
  const wizardOpenApiStatsDrift = useMemo(
    () =>
      buildEndpointCatalogOpenApiStatsDrift({
        schemaStats: wizardSchemaEndpointOpenApiStats,
        openApiStats,
      }),
    [openApiStats, wizardSchemaEndpointOpenApiStats],
  );
  const wizardEndpointCatalogDiagnostics = useMemo(
    () =>
      buildEndpointCatalogDiagnostics({
        coverage: wizardEndpointCoverageStats,
        unmappedCount: wizardUnmappedSchemaEndpointKeys.length,
        isOpenApiSyncAvailable: !openApiSpecQuery.isError,
      }),
    [
  ...
  const wizardEndpointIssueReasonOptions =
  ...
  const hasEndpointCatalogIssueReasonKeysMismatch = Boolean(
    ...
    && (
      wizardSchemaIssueReasonKeys.length !== agentSchemaIssueReasonKeys.length
  ...
  const hasWizardIssueReasonKeysVsOpenApiMismatch = Boolean(
    ...
    && (
      wizardSchemaIssueReasonKeys.length !== openApiSpecIssueReasonKeys.length
      || wizardSchemaIssueReasonKeys.some(
  ...
  const hasAgentIssueReasonKeysVsOpenApiMismatch = Boolean(
    ...
    && openApiSpecIssueReasonKeys
    && (
      agentSchemaIssueReasonKeys.length !== openApiSpecIssueReasonKeys.length
  ...
  const wizardEndpointCatalogPayload = useMemo(
    () =>
      ({
        ...
        diagnostics: {
          ...
          mismatch_count: wizardEndpointCatalogDiagnostics.mismatchCount,
          operation_fallback_count: wizardEndpointCatalogDiagnostics.fallbackCount,
          unmapped_endpoint_count: wizardEndpointCatalogDiagnostics.unmappedCount,
    ...
    [
      wizardEndpointCatalogDiagnostics.fallbackCount,
      wizardEndpointCatalogDiagnostics.isOpenApiSyncAvailable,
      wizardEndpointCatalogDiagnostics.issueCount,
      wizardEndpointCatalogDiagnostics.level,
  ...
  const wizardVisibleEndpointCatalogPayload = useMemo(
    () => {
      const visibleEndpointKeys = new Set(
        wizardVisibleSchemaEndpointsWithMethods.map((entry) => entry.key),
      );
      return {
        ...wizardEndpointCatalogPayload,
        endpoint_count_total: wizardEndpointCatalogPayload.endpoints.length,
        endpoint_count_visible: wizardVisibleSchemaEndpointsWithMethods.length,
        endpoints: wizardEndpointCatalogPayload.endpoints.filter((entry) =>
          visibleEndpointKeys.has(entry.key)
        ),
      };
    },
  ...
  const agentSchemaEndpointOpenApiStats = useMemo(
    () =>
      normalizeSchemaEndpointCatalogOpenApiStats(
        agentSchemaQuery.data?.endpoint_catalog_openapi_stats,
      ),
    [agentSchemaQuery.data?.endpoint_catalog_openapi_stats],
  );
  ...
  const agentOpenApiStatsDrift = useMemo(
    () =>
      buildEndpointCatalogOpenApiStatsDrift({
        schemaStats: agentSchemaEndpointOpenApiStats,
        openApiStats,
      }),
    [agentSchemaEndpointOpenApiStats, openApiStats],
  );
  const agentEndpointCatalogDiagnostics = useMemo(
    () =>
      buildEndpointCatalogDiagnostics({
        coverage: agentEndpointCoverageStats,
        unmappedCount: agentUnmappedSchemaEndpointKeys.length,
        isOpenApiSyncAvailable: !openApiSpecQuery.isError,
      }),
    [
      agentEndpointCoverageStats,
  ...
  const agentEndpointCatalogPayload = useMemo(
    () =>
      ({
        ...
        diagnostics: {
          ...
          mismatch_count: agentEndpointCatalogDiagnostics.mismatchCount,
          operation_fallback_count: agentEndpointCatalogDiagnostics.fallbackCount,
          unmapped_endpoint_count: agentEndpointCatalogDiagnostics.unmappedCount,
    ...
    [
      agentEndpointCatalogDiagnostics.fallbackCount,
      agentEndpointCatalogDiagnostics.isOpenApiSyncAvailable,
      agentEndpointCatalogDiagnostics.issueCount,
      agentEndpointCatalogDiagnostics.level,
  ...
  const agentVisibleEndpointCatalogPayload = useMemo(
    () => {
      const visibleEndpointKeys = new Set(
      ...
      return {
        ...
        endpoint_count_total: agentEndpointCatalogPayload.endpoints.length,
        endpoint_count_visible: agentVisibleSchemaEndpointsWithMethods.length,
        endpoints: agentEndpointCatalogPayload.endpoints.filter((entry) =>
          visibleEndpointKeys.has(entry.key)
        ),
      };
    },
  ...
  async function handleCopyVisibleWizardEndpointCatalog() {
    if (wizardVisibleEndpointCatalogPayload.endpoints.length === 0) {
      return;
    }
    setIsCopyingVisibleWizardEndpointCatalog(true);
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(
          {
            schema: "wizard",
            endpoint_count: wizardVisibleEndpointCatalogPayload.endpoints.length,
            scope: "visible",
            ...wizardVisibleEndpointCatalogPayload,
          },
          null,
          2,
        ),
      ...
      clearUiError();
      setNotice(
        `Copied visible wizard endpoint catalog metadata (${wizardVisibleEndpointCatalogPayload.endpoints.length} endpoints).`,
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible wizard endpoint catalog metadata.");
    } finally {
      setIsCopyingVisibleWizardEndpointCatalog(false);
    }
  }
  ...
  async function handleCopyVisibleWizardEndpointUrls() {
    if (wizardVisibleSchemaEndpointsWithMethods.length === 0) {
      return;
    }
    setIsCopyingVisibleWizardEndpointUrls(true);
    try {
      const payload = wizardVisibleSchemaEndpointsWithMethods
        .map((entry) =>
          `${entry.methods.length > 0 ? `[${entry.methods.join("|")}] ` : ""}${entry.endpoint}`
        )
        .join("\n");
      await navigator.clipboard.writeText(payload);
      clearUiError();
      setNotice(
        `Copied visible wizard endpoint URLs (${wizardVisibleSchemaEndpointsWithMethods.length}).`,
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible wizard endpoint URLs.");
    } finally {
      setIsCopyingVisibleWizardEndpointUrls(false);
    }
  }
  ...
  async function handleCopyVisibleWizardEndpointIssueReport() {
    if (wizardVisibleEndpointIssueReportPayload.issue_entries.length === 0) {
      return;
    }
    setIsCopyingVisibleWizardEndpointIssueReport(true);
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(
          {
            schema: "wizard",
            scope: "visible-issues",
            ...wizardVisibleEndpointIssueReportPayload,
          },
          null,
          2,
      ...
      clearUiError();
      setNotice(
        `Copied visible wizard endpoint issue report (${wizardVisibleEndpointIssueReportPayload.issue_entries.length} issues).`,
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible wizard endpoint issue report.");
    } finally {
      setIsCopyingVisibleWizardEndpointIssueReport(false);
    }
  }

  async function handleCopyVisibleWizardIssueKeys() {
    if (wizardVisibleIssueKeys.length === 0) {
      return;
    }
    setIsCopyingVisibleWizardIssueKeys(true);
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(
          {
      ...
      clearUiError();
      setNotice(`Copied visible wizard issue keys (${wizardVisibleIssueKeys.length}).`);
    } catch (error) {
      applyUiError(error, "Failed to copy visible wizard issue keys.");
    } finally {
      setIsCopyingVisibleWizardIssueKeys(false);
    }
  }
  ...
  async function handleCopyVisibleWizardIssueReasonCounts() {
    if (wizardVisibleEndpointIssueReportPayload.issue_entries.length === 0) {
      return;
    }
    setIsCopyingVisibleWizardIssueReasonCounts(true);
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(
          {
            schema: "wizard",
            scope: "visible-issue-reason-counts",
            view_mode: wizardEndpointCatalogViewMode,
            issue_reason_filter: wizardEndpointCatalogIssueReasonFilter,
            filter: wizardEndpointCatalogFilter.trim() || null,
            sort: wizardEndpointCatalogSort,
            visible_issue_count: wizardVisibleEndpointIssueReportPayload.issue_entries.length,
            issue_reason_counts:
              wizardVisibleEndpointIssueReportPayload.issue_reason_counts_visible,
          },
          null,
      ...
      clearUiError();
      setNotice(
        "Copied visible wizard issue reason counts.",
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible wizard issue reason counts.");
    } finally {
      setIsCopyingVisibleWizardIssueReasonCounts(false);
    }
  }
  ...
  async function handleCopyVisibleWizardIssueSummaries() {
    ...
    setIsCopyingVisibleWizardIssueSummaries(true);
    try {
      const payload = wizardVisibleEndpointIssueReportPayload.issue_entries
        .map((entry) => `${entry.key}: ${entry.summary ?? "∅"} (${entry.reasons.join(", ")})`)
        .join("\n");
      await navigator.clipboard.writeText(payload);
      clearUiError();
      setNotice(
        `Copied visible wizard issue summaries (${wizardVisibleEndpointIssueReportPayload.issue_entries.length}).`,
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible wizard issue summaries.");
    } finally {
      setIsCopyingVisibleWizardIssueSummaries(false);
    }
  }

  async function handleCopyVisibleWizardIssuePaths() {
    if (wizardVisibleEndpointIssueReportPayload.issue_entries.length === 0) {
      return;
    }
    setIsCopyingVisibleWizardIssuePaths(true);
    try {
      const payload = wizardVisibleEndpointIssueReportPayload.issue_entries
        .map((entry) => `${entry.key}: ${entry.openapi_path} (${entry.reasons.join(", ")})`)
        .join("\n");
      await navigator.clipboard.writeText(payload);
      clearUiError();
      setNotice(
        `Copied visible wizard issue OpenAPI paths (${wizardVisibleEndpointIssueReportPayload.issue_entries.length}).`,
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible wizard issue OpenAPI paths.");
    } finally {
      setIsCopyingVisibleWizardIssuePaths(false);
    }
  }
  ...
  async function handleCopyVisibleAgentEndpointCatalog() {
    if (agentVisibleEndpointCatalogPayload.endpoints.length === 0) {
      return;
    }
    setIsCopyingVisibleAgentEndpointCatalog(true);
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(
          {
            schema: "agent",
            endpoint_count: agentVisibleEndpointCatalogPayload.endpoints.length,
            scope: "visible",
            ...agentVisibleEndpointCatalogPayload,
          },
          null,
          2,
        ),
      ...
      clearUiError();
      setNotice(
        `Copied visible agent endpoint catalog metadata (${agentVisibleEndpointCatalogPayload.endpoints.length} endpoints).`,
      );
    } catch (error) {
  ...
  async function handleCopyVisibleAgentEndpointUrls() {
    if (agentVisibleSchemaEndpointsWithMethods.length === 0) {
      return;
    }
    setIsCopyingVisibleAgentEndpointUrls(true);
    try {
      ...
      clearUiError();
      setNotice(
        `Copied visible agent endpoint URLs (${agentVisibleSchemaEndpointsWithMethods.length}).`,
      );
    } catch (error) {
  ...
  async function handleCopyVisibleAgentEndpointIssueReport() {
    if (agentVisibleEndpointIssueReportPayload.issue_entries.length === 0) {
      return;
    }
    setIsCopyingVisibleAgentEndpointIssueReport(true);
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(
          {
      ...
      clearUiError();
      setNotice(
        `Copied visible agent endpoint issue report (${agentVisibleEndpointIssueReportPayload.issue_entries.length} issues).`,
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible agent endpoint issue report.");
    } finally {
      setIsCopyingVisibleAgentEndpointIssueReport(false);
    }
  }

  async function handleCopyVisibleAgentIssueKeys() {
    if (agentVisibleIssueKeys.length === 0) {
      return;
    }
    setIsCopyingVisibleAgentIssueKeys(true);
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(
          {
      ...
      clearUiError();
      setNotice(`Copied visible agent issue keys (${agentVisibleIssueKeys.length}).`);
    } catch (error) {
  ...
  async function handleCopyVisibleAgentIssueReasonCounts() {
    if (agentVisibleEndpointIssueReportPayload.issue_entries.length === 0) {
      return;
    }
    setIsCopyingVisibleAgentIssueReasonCounts(true);
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(
          {
            ...
            filter: agentEndpointCatalogFilter.trim() || null,
            sort: agentEndpointCatalogSort,
            visible_issue_count: agentVisibleEndpointIssueReportPayload.issue_entries.length,
            issue_reason_counts:
              agentVisibleEndpointIssueReportPayload.issue_reason_counts_visible,
          },
          null,
      ...
      clearUiError();
      setNotice(
        "Copied visible agent issue reason counts.",
      );
    } catch (error) {
  ...
  async function handleCopyVisibleAgentIssueSummaries() {
    if (agentVisibleEndpointIssueReportPayload.issue_entries.length === 0) {
      return;
    }
    setIsCopyingVisibleAgentIssueSummaries(true);
    try {
      const payload = agentVisibleEndpointIssueReportPayload.issue_entries
        .map((entry) => `${entry.key}: ${entry.summary ?? "∅"} (${entry.reasons.join(", ")})`)
        .join("\n");
      ...
      clearUiError();
      setNotice(
        `Copied visible agent issue summaries (${agentVisibleEndpointIssueReportPayload.issue_entries.length}).`,
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible agent issue summaries.");
    } finally {
      setIsCopyingVisibleAgentIssueSummaries(false);
    }
  }

  async function handleCopyVisibleAgentIssuePaths() {
    if (agentVisibleEndpointIssueReportPayload.issue_entries.length === 0) {
      return;
    }
    setIsCopyingVisibleAgentIssuePaths(true);
    try {
      const payload = agentVisibleEndpointIssueReportPayload.issue_entries
        .map((entry) => `${entry.key}: ${entry.openapi_path} (${entry.reasons.join(", ")})`)
        .join("\n");
      ...
      clearUiError();
      setNotice(
        `Copied visible agent issue OpenAPI paths (${agentVisibleEndpointIssueReportPayload.issue_entries.length}).`,
      );
    } catch (error) {
      applyUiError(error, "Failed to copy visible agent issue OpenAPI paths.");
    } finally {
      setIsCopyingVisibleAgentIssuePaths(false);
    }
  }
              ...
              <details className="mb-2 rounded border border-slate-800 bg-slate-950/60 p-2">
                ...
                <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    ...
                    <button
                      onClick={handleCopyVisibleWizardEndpointCatalog}
                      disabled={isCopyingVisibleWizardEndpointCatalog}
                    ...
                    <button
                      onClick={handleCopyVisibleWizardEndpointUrls}
                      disabled={isCopyingVisibleWizardEndpointUrls}
                      className="rounded border border-slate-700 px-2 py-0.5 text-[10px] text-slate-300 hover:bg-slate-800 disabled:opacity-40"
                    >
                    ...
                    <button
                      onClick={handleCopyVisibleWizardEndpointIssueReport}
                      disabled={
                        isCopyingVisibleWizardEndpointIssueReport
                    ...
                    <button
                      onClick={handleCopyVisibleWizardIssueKeys}
                      disabled={isCopyingVisibleWizardIssueKeys || wizardVisibleIssueKeys.length === 0}
                      className="rounded border border-amber-600/60 px-2 py-0.5 text-[10px] text-amber-200 hover:bg-amber-500/10 disabled:opacity-40"
                    >
                    ...
                    <button
                      onClick={handleCopyVisibleWizardIssuePaths}
                      disabled={
                        isCopyingVisibleWizardIssuePaths
                        || wizardVisibleEndpointIssueReportPayload.issue_entries.length === 0
                      }
                      className="rounded border border-amber-600/60 px-2 py-0.5 text-[10px] text-amber-200 hover:bg-amber-500/10 disabled:opacity-40"
                    >
                      {isCopyingVisibleWizardIssuePaths
                        ? "Copying..."
                        : "Copy visible issue paths"}
                    </button>
                    <button
                      onClick={handleCopyVisibleWizardIssueReasonCounts}
                      disabled={
                    ...
                    <button
                      ...
                      disabled={
                        isCopyingVisibleWizardIssueSummaries
                        || wizardVisibleEndpointIssueReportPayload.issue_entries.length === 0
                      }
                      className="rounded border border-amber-600/60 px-2 py-0.5 text-[10px] text-amber-200 hover:bg-amber-500/10 disabled:opacity-40"
                    >
                ...
                {wizardSchemaEndpointOpenApiStats ? (
                  <p className="mt-1 text-[11px] text-slate-500">
                    schema openapi stats: paths{" "}
                    <span className="font-mono text-slate-300">
                      {wizardSchemaEndpointOpenApiStats.pathCount}
                    </span>
                    , operations{" "}
                    <span className="font-mono text-slate-300">
                      {wizardSchemaEndpointOpenApiStats.operationCount}
                    </span>
                    , summarized{" "}
                    <span className="font-mono text-slate-300">
                      {wizardSchemaEndpointOpenApiStats.summarizedOperationCount}
                    </span>
                  </p>
                ) : (
                  <p className="mt-1 text-[11px] text-amber-300">
                    schema endpoint_catalog_openapi_stats metadata missing.
                  </p>
                )}
                {wizardOpenApiStatsDrift?.hasDrift ? (
                  <p className="mt-1 text-[11px] text-rose-300">
                    schema/openapi stats drift:{" "}
                    <span className="font-mono">
                      {[
                        wizardOpenApiStatsDrift.pathCount ? "path_count" : null,
                        wizardOpenApiStatsDrift.operationCount ? "operation_count" : null,
                        wizardOpenApiStatsDrift.summarizedOperationCount
                  ...
                  <p className="mt-1 text-[11px] text-emerald-300">
                    schema/openapi stats are in sync.
                  </p>
                ) : null}
                {hasWizardSchemaCoverageInSync ? (
                  <p className="mt-1 text-[11px] text-emerald-300">
                ...
                {wizardSchemaIssueReasonKeys ? (
                  <p className="mt-1 text-[11px] text-slate-500">
                    issue reason keys (schema):{" "}
                    <span className="font-mono text-slate-300">
                      {wizardSchemaIssueReasonKeys.join(", ")}
                    </span>
                  </p>
                ) : null}
                {openApiSpecIssueReasonKeys ? (
                  <p className="mt-1 text-[11px] text-slate-500">
                    issue reason keys (/v1/openapi extension):{" "}
                    <span className="font-mono text-slate-300">
                      {openApiSpecIssueReasonKeys.join(", ")}
                    </span>
                  </p>
                ) : null}
                {hasEndpointCatalogIssueReasonKeysMismatch ? (
                  <p className="mt-1 text-[11px] text-rose-300">
                    endpoint catalog issue reason keys mismatch between wizard and agent schemas.
                  </p>
                ) : wizardSchemaIssueReasonKeys ? (
                  <p className="mt-1 text-[11px] text-emerald-300">
                ...
                {hasWizardIssueReasonKeysVsOpenApiMismatch ? (
                  ...
                  wizardSchemaIssueReasonKeys && openApiSpecIssueReasonKeys
                    ? (
                      <p className="mt-1 text-[11px] text-emerald-300">
              ...
              <details className="mb-2 rounded border border-slate-800 bg-slate-950/60 p-2">
                ...
                <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    ...
                    <button
                      onClick={handleCopyVisibleAgentEndpointCatalog}
                      disabled={isCopyingVisibleAgentEndpointCatalog}
                      className="rounded border border-slate-700 px-2 py-0.5 text-[10px] text-slate-300 hover:bg-slate-800 disabled:opacity-40"
                    >
                    ...
                    <button
                      onClick={handleCopyVisibleAgentEndpointUrls}
                      disabled={isCopyingVisibleAgentEndpointUrls}
                      className="rounded border border-slate-700 px-2 py-0.5 text-[10px] text-slate-300 hover:bg-slate-800 disabled:opacity-40"
                    >
                    ...
                    <button
                      onClick={handleCopyVisibleAgentEndpointIssueReport}
                      disabled={
                        isCopyingVisibleAgentEndpointIssueReport
                    ...
                    <button
                      onClick={handleCopyVisibleAgentIssueKeys}
                      disabled={isCopyingVisibleAgentIssueKeys || agentVisibleIssueKeys.length === 0}
                      className="rounded border border-amber-600/60 px-2 py-0.5 text-[10px] text-amber-200 hover:bg-amber-500/10 disabled:opacity-40"
                    >
                    ...
                    <button
                      onClick={handleCopyVisibleAgentIssuePaths}
                      disabled={
                        isCopyingVisibleAgentIssuePaths
                        || agentVisibleEndpointIssueReportPayload.issue_entries.length === 0
                      }
                      className="rounded border border-amber-600/60 px-2 py-0.5 text-[10px] text-amber-200 hover:bg-amber-500/10 disabled:opacity-40"
                    >
                    ...
                    <button
                      onClick={handleCopyVisibleAgentIssueReasonCounts}
                      disabled={
                    ...
                    <button
                      ...
                      disabled={
                        isCopyingVisibleAgentIssueSummaries
                        || agentVisibleEndpointIssueReportPayload.issue_entries.length === 0
                      }
                      className="rounded border border-amber-600/60 px-2 py-0.5 text-[10px] text-amber-200 hover:bg-amber-500/10 disabled:opacity-40"
                    >
                ...
                {agentSchemaEndpointOpenApiStats ? (
                  <p className="mt-1 text-xs text-slate-500">
                    schema openapi stats: paths{" "}
                    <span className="font-mono text-slate-300">
                      {agentSchemaEndpointOpenApiStats.pathCount}
                    </span>
                    , operations{" "}
                    <span className="font-mono text-slate-300">
                      {agentSchemaEndpointOpenApiStats.operationCount}
                    </span>
                    , summarized{" "}
                    <span className="font-mono text-slate-300">
                      {agentSchemaEndpointOpenApiStats.summarizedOperationCount}
                    </span>
                  </p>
                ) : (
                  <p className="mt-1 text-xs text-amber-300">
                ...
                {agentOpenApiStatsDrift?.hasDrift ? (
                  <p className="mt-1 text-xs text-rose-300">
                    ...
                    <span className="font-mono">
                      {[
                        agentOpenApiStatsDrift.pathCount ? "path_count" : null,
                        agentOpenApiStatsDrift.operationCount ? "operation_count" : null,
                        agentOpenApiStatsDrift.summarizedOperationCount
                          ? "summarized_operation_count"
                          : null,
                      ]
                        .filter((value): value is string => Boolean(value))
                ...
                {agentSchemaIssueReasonKeys ? (
                  <p className="mt-1 text-xs text-slate-500">
                    issue reason keys (schema):{" "}
                    <span className="font-mono text-slate-300">
                      {agentSchemaIssueReasonKeys.join(", ")}
                    </span>
                  </p>
                ) : null}
                {openApiSpecIssueReasonKeys ? (
                ...
                {hasAgentIssueReasonKeysVsOpenApiMismatch ? (
                  ...
                  agentSchemaIssueReasonKeys && openApiSpecIssueReasonKeys
                    ? (
                      <p className="mt-1 text-xs text-emerald-300">
                        endpoint catalog issue reason keys match /v1/openapi extension.
                      </p>
